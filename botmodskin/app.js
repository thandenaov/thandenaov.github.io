...
function openLetter(L) {
  ...
  $('heroGrid').innerHTML = folders.map((f, i) => {
    ...
    return `<button class="hero-cell hero-card${state.cart[f] ? ' has-skin' : ''}"
      ...
      <div class="hc-meta">
        <span class="hc-name">${escapeHtml(f)}<small class="hc-inline">${state.cart[f] ? `› ${escapeHtml(String(state.cart[f]).replace(f + ' ', ''))}` : `${skins.length} skin`}</small></span>
      </div>
...
function pickSkin(folder, skin, cellEl) {
  ...
  updateBadge();
  setTimeout(() => { openLetter(state.currentLetter || folder[0].toUpperCase()); }, 180);
}
